...
contract MerchantBackend is ExpiryHelper {
    ...
    event Response(int256 response); // borrowing/returning responses and giving REP tokens to the customer

    address public ftAddress; // address of our fungible token collection (REP)
    address public owner; // address of the account used to deploy this smart contract
    uint256 public lockupAmount = 100000000000; // 1000HBAR; amount that a customer is required to lock up for the duration of the lending

    constructor() payable {
        // Creating the fungible token collection (REP)
        IHederaTokenService.HederaToken memory token;
        token.name = "Reputation Tokens";
        token.symbol = "REP";
        token.memo = "REP Tokens By: Aniket";
        token.treasury = address(this);
        token.expiry = createAutoRenewExpiry(address(this), 7000000);

        (int256 responseCode, address tokenAddress) = HederaTokenService
            .createFungibleToken(token, 1000, 0); // (token, initial supply, decimals)
        ...
        payable
    {
        ...
        token.name = name;
        token.symbol = symbol;
        token.memo = "CAR Collection By: Aniket";
        token.treasury = address(this);
    ...
    // Giving REP (Reputation tokens) to the customer
